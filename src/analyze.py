...
def to_cell_space(spikes):
    ...
    for index_1, neuron_1 in enumerate(data):
        for index_2, neuron_2 in enumerate(data):
            data[index_1, index_2] = np.exp(
                -correlator(spikes[index_1], spikes[index_2])
            )
    return data
...
def make_model(
    ...
    data_model = {}
    data_model[model_name] = grating_model(
        Nn=Nn,
        Np=(Np_or, Np_fr, Np_ph, Np_co),
...
def analyze_data(
    data_list,
    orfrphco=[True, True, True, True],
    PCA_dim=None,
    cell_space=False,
    noise_reduction_fraction=None,
    noise_reduction_steps=100,
    z_cutoff=None,
    coeff=None,
    homdim=1,
    decode=None,
    ...
    for data in data_list:
        ...
        if noise_reduction_fraction is not None:
            data = top_noise_reduction(
                data,
                n=noise_reduction_steps,
                omega=0.6,
                fraction=noise_reduction_fraction,
            )
        ...
        if z_cutoff is not None:
            ...
            plot_data(
                data,
                transformation=plot_transformation,
                labels=z_labels,
        ...
        if decode is not None:
            if decode == "Cohomology":
                decoding = cohomological_decoding(
                    data,
                    coeff=[coeff, 23][coeff is None],
                    weighted=weighted_decoding,
                    cocycle_number=decode_feature,
                    name=name,
                    save_path=save_path_data,
                )
            elif decode == "SpectralEmbedding":
                decoding = manifold.SpectralEmbedding(
                    n_components=1, affinity="rbf"
                ).fit_transform(data)
            elif decode == "PCA":
            ...
            plot_data(
                ...
                colors=[
                    ...
                    "Viridis",
                    "Twilight",
                    ["Viridis", "Twilight"][decode == "Cohomology"],
                ],
                save_path=save_path_data,
            )
            ...
            reduced_data[name + "_feature" + str(decode_feature) + "removed"] = (
                remove_feature(data, decoding, shift=shift)
            )
        else:
            plot_data(
                data,
                transformation=plot_transformation,
                colors=["Twilight", "Viridis", "Viridis", "Twilight"],
                save_path=save_path_data,
            )
...
def receptive_fields(data, decodingx, decodingy):
    ...
    for neuron in data:
        ...
        interpolated_data = scipy.interpolate.griddata(
            pd.concat([decodingx, decodingy], axis=1), data[neuron], (xv, yv)
        )
